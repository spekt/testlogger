...
namespace Spekt.TestLogger.UnitTests.Extensions
{
    ...
    public class MSTestAdapterTests
    {
        ...
        {
            var builder = new TestResultInfoBuilder("namespace", "type", Method)
                .WithProperty("Microsoft.VisualStudio.TestTools.UnitTesting.TestContext.TestProperty", new[] { "c1", "c2" });

            var testResults = new List<TestResultInfo> { builder.Build() };

            var sut = new MSTestAdapter();

            var transformedResults = sut.TransformResults(testResults, new List<TestMessageInfo>(), builder.TestCase);
